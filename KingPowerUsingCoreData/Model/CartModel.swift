...
public class CartModel{
    ...
    var queryInsertCart : String = "INSERT INTO CART (CART_ID, CART_USER_ID, CART_CUST_ID, CART_PROD_ID, CART_QUANTITY, CART_PICKUP_NOW, CART_CURRENT_LOCATION, CART_CREATE_DATE, CART_UPDATE_DATE) VALUES (%d, %d, %d, %d, %d, '%@', '%@', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);"
    var queryUpdatecartById : String = "UPDATE CART SET CART_QUANTITY = %d, CART_PICKUP_NOW = '%@', CART_UPDATE_DATE = CURRENT_TIMESTAMP WHERE CART_ID = %d;"
    var queryDeleteCartById : String = "DELETE FROM CART WHERE CART_CUST_ID = %d AND CART_USER_ID = %d;"
    var queryDeleteCartByCartId : String = "DELETE FROM CART WHERE CART_ID = %d;"
    ...
    var queryGetMaxId : String = "SELECT MAX(CART_ID) as max_col FROM CART"
}